...
except ImportError:  # [VENDOR PATCH] canonical poker44 lacks this helper; inline fallback
    import re as _re
    from pathlib import Path as _Path
    _ARTIFACT_VERSION_RE = _re.compile(r"v(\d+(?:[._-]\d+)*)")
    def artifact_model_identity(artifact_path):
        p = _Path(artifact_path); stem = (p.stem or "poker44-model").strip()
        m = _ARTIFACT_VERSION_RE.search(stem)
        return {"model_name": stem, "model_version": m.group(1) if m else "1",
