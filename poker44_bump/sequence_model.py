# Vendored from phasberg/poker44-model-v5 (MIT License, (c) 2025 Poker44 Contributors)
# Source: https://github.com/phasberg/poker44-model-v5  poker44_ml/sequence_model.py
# ChunkSetTransformer hierarchical-attention chunk learner. Unmodified except this header.
...
class ChunkSetTransformer(nn.Module):
    ...
    def __init__(self, config: SequenceModelConfig) -> None:
        ...
        self.action_encoder = nn.TransformerEncoder(
            ...
            enable_nested_tensor=False,   # [VENDOR PATCH] avoid nested-tensor NaN on padded rows
        )
    ...
    def forward(
        ...
        # [VENDOR PATCH] NaN-safety: nn.TransformerEncoder yields NaN for
        # fully-padded rows (empty padded hands / padded chunk slots). Unmask
        # col 0 for the encoder pass; the AttentionPool + hand_mask zero these
        # rows downstream, so their encoder output is irrelevant. Without this
        # the loss diverges (all probs -> 1.0 -> NaN).
        key_padding = ~flat_action_mask
        enc_kp = key_padding.clone()
        _full_a = enc_kp.all(dim=1)
        if _full_a.any():
            enc_kp[_full_a, 0] = False
        encoded = self.action_encoder(embed, src_key_padding_mask=enc_kp)
        ...
        hand_kp = ~hand_mask
        hand_kp_safe = hand_kp.clone()
        _full_h = hand_kp_safe.all(dim=1)
        if _full_h.any():
            hand_kp_safe[_full_h, 0] = False
        encoded_hands = self.hand_encoder(hand_emb, src_key_padding_mask=hand_kp_safe)
        encoded_hands = torch.nan_to_num(encoded_hands)
    ...
    def fit(
        ...
        for epoch in range(int(self.n_epochs)):
            ...
            if self.verbose_metrics and len(val_chunks) > 0:
                from poker44_bump.chunk_score_metrics import print_chunk_score_diagnostics
